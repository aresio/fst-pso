...
def process_operators(operator, verbose=False):
	if isinstance(operator, fuzzy.operator.Input.Input):
		if verbose: print("    Simple input", operator.adjective.getName(fuzzySystem), end=' ') 
...
def return_all_output_variable_names(fuzzySystem):
	ret = {}
	for name, i in list(fuzzySystem.variables.items()):
...
def actual_sugeno_inference(fuzzySystem, verbose=False):
	...
	for name, rule in list(fuzzySystem.rules.items()):
		...
		if isinstance(rule.adjective, fuzzy.Adjective.Adjective):
			...
			for adj in rule.adjective:
				...
				if verbose: print("   ", outlabel, outname, end=' ')
	...
	final = {}
	for out, valout in list(all_outputs.items()):
		if valout[1]==0:
			final[out]=0
		else:
